...
def get_contents(filename):
    ...
    with bz2.open(filename, 'rb') as f:
        for line in f:
            ...
            #每个文章title的第一段落的每个句子，抽取出其ner，包含ner_text,ner在该句子的开始结束位置，以及ner的类型
            _text_ner = []
            for sent in doc['text']:
                ent_list = [(ent.text, ent.start_char, ent.end_char, ent.label_) for ent in nlp(sent).ents] #nlp()处理text,返回的是Doc对象
