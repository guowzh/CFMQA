...
def extract_ner_from_titles(sent, titles, context_ners=None):
    ...
    # select candidates for question from all NER in context **if ner in context appears in question ,then select the 
    candidates = set()
...
#train/dev的json文件，对于每条question,将问题中的ner提取出来，构造question_guid2nerlist
def extract_question_ner(full_data):
    ...
    for idx, doc in enumerate(nlp.pipe(all_questions, n_process=16, batch_size=1000)):  #nlp.pipe对文本列表里的每个文本生成一个Doc对象
...
#train/dev的json文件，其中每个QA实例包含_id:str,context:[[title1,[sent1,sent2]],...] ,questio,answer...，从context中抽取ner
#构造成context_guid2nerlists,抽取每个句子的ner
    ...
    for case in full_data:
        ...
        for title, sents in case['context']:
            ...
            for sent, sent_ner in zip(sents, ner_data[title]['text_ner']):
                ...
                # optional
                context_ner[-1].extend(extract_ner_from_titles(sent, titles))   #该句子可能包含有title，即指向其他title
...
print("extract ners from questions and context is done!")