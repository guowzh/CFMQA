...
def convert_examples_to_features(examples, tokenizer, max_seq_length, max_query_length, max_entity_num,
    ...
    for (example_index, example) in enumerate(tqdm(examples)):
        ...
        if len(all_doc_tokens) > max_seq_length:
            longers.append(len(all_doc_tokens)) 
        ...
        if sent_max_index < len(sentence_spans):
            sentence_spans = sentence_spans[:sent_max_index] if sent_max_index > 0 else [(sentence_spans[0][0],max_seq_length-1)]
    ...
    print(failed)
    print("truncated count:",len(longers))
    # print(longers)
    return features
