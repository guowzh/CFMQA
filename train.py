...
# dev_example_dict = None
# dev_feature_dict = None
# dev_dataloader = None
#########################################################################
# Initialize Model
##########################################################################
...
for epoch in train_iterator:
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        batch['constrast_encoding'] = encoder(**contrast_inputs)[0] if args.counterfactual else None
        batch['perturbed_encoding'] = encoder(**perturbed_inputs)[0] if args.counterfactual else None
        batch['context_mask'] = batch['context_mask'].float().to(args.device)
        results = model(batch, return_yp=True)

        loss_list = cf_compute_loss(args, batch, results)
        del batch
